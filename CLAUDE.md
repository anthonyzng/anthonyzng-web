...
- **Where**: GCP project `anthonyzng-web` (billing linked), VM `anthonyzng-web` in `us-east1-b`: e2-micro (free tier; 1 GiB RAM plus 2 GiB swap), Debian 12, 30 GB standard disk, Shielded VM, network tag `web`; static external IPv4 `anthonyzng-web-ip` = `34.26.232.31` (a static address costs the same as an ephemeral one while in use, and DNS never has to change). Firewall (`default` network): `allow-web` (tcp 80, 443, udp 443 → tag `web`), `default-allow-ssh` (22, key-only logins), ICMP and internal; the default RDP rule was deleted. The owner's gcloud CLI is signed in on the dev machine (`gcloud config`: project `anthonyzng-web`, zone `us-east1-b`); `gcloud compute ssh anthonyzng-web` gives an admin shell.
- **VM setup** (once, `deploy/vm-setup.sh`, run as root with the deploy public key): Docker Engine + Compose plugin from Docker's apt repository, 2 GiB swap (`vm.swappiness=10`), the `local` log driver (10 MB × 3 per container), unattended security upgrades, the `deploy` user (no password, key login only, in the `docker` group) and `/opt/anthonyzng-web` (`750`, owned by `deploy`) with `backend/` and `proxy/`. The VM's SSH host key is pinned in the `DEPLOY_KNOWN_HOSTS` secret (ED25519 `SHA256:pY1Yg2Z6ho87hgcoEHiFYYqIpJTOOgtjyLkU7MZa/pY`, cross-checked over the gcloud channel and a fresh scan).
