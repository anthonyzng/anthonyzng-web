...
6. Frontend build-time variables (`frontend/.env.example`) must reach the **image build**, not the running container: pass them as Docker build args and GitHub Actions secrets. `VITE_CONTACT_EMAIL` is required for a real deploy — without it the Contact section ships with no email row, and the production build warns.
