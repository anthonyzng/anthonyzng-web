 ...
 * row parts around the centred `[data-zipper-title]` as it passes: its label slides left and its
      ...
      mm.add(
        ...
        (ctx) => {
          ...
          const local = runSafely(() => {
            ...
            const title = root.querySelector<HTMLElement>('[data-zipper-title] > span')
            ...
            const render = () => {
              ...
              rows.forEach((_, i) => {
                ...
                setY[i](y - natural)
                const distance = y + pitch / 2 - height / 2
